...
class AreaModel extends Model
{
    ...
    public function insertArea($param)
    {
        try{
            ...
            //$this->validate('AreaValidate')->allowField(true)->insert($param)
            $result = $this->validate('AreaValidate')->strict(false)->insertGetId($param);
            if(false === $result){            
                return ['code' => -1, 'data' => '', 'msg' => $this->getError()];
            }else{
                if (Plugins::where(['name'=>'areaextend'])->find()) {
                    $plugClass = get_plugins_class('areaextend');
                    if (class_exists($plugClass)) {
                        $plugObj = new $plugClass;
                        $plugObj->updatedata($result, $param);
                    }
                }
    ...
    public function editArea($param)
    {
        try{
            ...
            $result =  $this->validate('AreaValidate')->allowField(true)->save($param, ['id' => $param['id']]);
            if(false === $result){            
                ...
                if (Plugins::where(['name'=>'areaextend'])->find()) {
                    ...
                    if (class_exists($plugClass)) {
                        $plugObj = new $plugClass;
                        $plugObj->updatedata($param['id'], $param);
                    }
                }
