...
class ContentModel extends Model
{
    ...
    public function getContentArea($con, $area = [])
    {
        ...
        if ($area) {
            ...
            $misarea = $categorymodel->where(['id'=>$con['cid']])->value('areacontitle');
            $con['alltitle'] = isset($con['alltitle']) ? $con['alltitle'] : $con['title'];
    ...
    public function getTagnum($tagname){
        $where['create_time'] = ['lt',time()];
        $where['tag'] = ['neq',''];
    ...
    public function getContentUrl($con, $cw = '', $area = [], $openarea = true) {
	    ...
	    if (!empty($con['jumpurl'])) {
	        if (substr($con['jumpurl'], 0,1) == '@') {
                $cate = $categorymodel->where(['id'=>substr($con['jumpurl'], 1)])->find();
                $cateurl = $categorymodel->getCategoryUrl($cate);
                return $cateurl;
            }else{
                return $con['jumpurl'];
            }
	    }
	    $cate = $categorymodel->field('id,etitle')->where(['id'=>$con['cid']])->find();
    ...
    public function getContentlist($param){
        ...
        if ($param['sql'] != -1) {
            ...
            $wheresql = $sqlstr != ' ' ? $sqlstr : $wheresql;
        }
