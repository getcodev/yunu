...
class Buildhtml extends Common
{
	public function htmlmcontent(){
		if(request()->isAjax()){
	        ...
	        //纯静态主站内容
	        if ($htmltype == "staticmaincontent") {
	            $data = $this->handelBuildStaticArticle($fid, $achievepage);
	        }
	        //分站首页
	...
	//生成详情页
	private function handelBuildStaticArticle($fid, $achievepage = 0, $batch = true, $limit = 30){
        function_exists('set_time_limit') && set_time_limit(0);
        ...
        foreach ($conlist as $k => $v) {
            if ($v['jumpurl'] == "") {
                curl(config('sys.site_protocol')."://".config('sys.site_url')."/index.php?s=index/show/index&id=".$v['id']."&publichtml=1"); 
            }
            $data['achievepage'] +=  1;
        }
        $data['fid'] = $fid + 1;
